...
def _get_submodules_old(package_name: str) -> list[str]:
    ...
    """
    print(f"Loading submodules for {package_name}")
    try:
...
def _get_submodules(module_name: str, package_name: str) -> list[str]:
    """Get all submodules of a package.
...
def _import_submodules(module_name: str, package_name: str) -> Optional[List[ModuleType]]:
    ...
    submodules_names = _get_submodules(module_name=module_name, package_name=package_name)
    modules = [_import_module(n) for n in submodules_names]
...
def _import_all_members(module_name: str, package_name: str) -> List[str]:
    submodules = _import_submodules(module_name=module_name, package_name=package_name)
...
def _get_submodule_members(module_name: str, package_name: str) -> List[str]:
    ...
    """
    members = _import_all_members(module_name=module_name, package_name=package_name)
...
def _generate_api_docs_for_module(root_path: Path, module_name: str, package_name: str) -> str:
    ...
    members_with_submodules = _get_submodule_members(module_name=module_name, package_name=package_name)
    symbols = _load_submodules(module_name=module_name, package_name=package_name, members_with_submodules=members_with_submodules)

    _update_api_docs(symbols, root_path, module_name)

    # todo: fix the problem and remove this
    src = """                    - [ContactDict](api/monotonic/asyncapi/schema/info/ContactDict.md)
"""
    dst = """                    - [ContactDict](api/monotonic/asyncapi/schema/info/ContactDict.md)
                    - [EmailStr](api/monotonic/asyncapi/schema/info/EmailStr.md)
"""
...
def create_api_docs(
    ...
    """
    api = _generate_api_docs_for_module(root_path, module_name=module_name, package_name=package_name)
...
if __name__ == "__main__":
    ...
    create_api_docs(root, package_name="monotonic-nn", module_name="monotonic")

...
def get_activation_functions(
    ...
    return convex_activation, concave_activation
...
def apply_activations(
    ...
    *,
    ...
    activation_weights: tuple[float, float] = (1.0, 1.0),
) -> TensorLike:
    if convex_activation is None:
        return x
    elif is_convex:
        normalized_activation_weights = np.array([1.0, 0.0])
    elif is_concave:
        normalized_activation_weights = np.array([0.0, 1.0])
    else:
        if len(activation_weights) != 2:
    ...
    s_concave = units - s_convex

    x_convex, x_concave = tf.split(x, (s_convex, s_concave), axis=-1)

    y_convex = convex_activation(x_convex)
    y_concave = concave_activation(x_concave)

    y = tf.concat([y_convex, y_concave], axis=-1)
